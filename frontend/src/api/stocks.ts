...
export async function searchSymbols(token: string, q: string) {
  const { data } = await http.get(`/api/stocks/search?q=${encodeURIComponent(q)}`, auth(token));
...
export async function createPosition(token: string, body: {
  ...
  const { data } = await http.post("/api/stocks/positions", body, auth(token));
  return data as PositionResp;
}

export async function listPositions(token: string) {
  const { data } = await http.get("/api/stocks/positions", auth(token));
...
export async function updatePosition(token: string, id: string, body: Partial<{ symbol: string; amountInvested: number; purchaseDate: string; }>) {
  const { data } = await http.put(`/api/stocks/positions/${id}`, body, auth(token));
  return data as PositionResp;
}

export async function deletePosition(token: string, id: string) {
  const { data } = await http.delete(`/api/stocks/positions/${id}`, auth(token));
  return data as { message: string; id: string };
}


export async function liveQuotes(token: string) {
  const { data } = await http.get("/api/stocks/quotes", auth(token));
...
export async function fetchHistory(
  ...
  const { data } = await http.get<HistoryResp>(`/api/stocks/history?${qs.toString()}`, auth(token));
  return data;
}
